...
Future<void> _eliminarServicio(Servicio servicio) async {
  ...
  try {
    final servicioProvider = Provider.of<ServicioProvider>(context, listen: false);
    await servicioProvider.deleteService(servicio.idServicio.toString());
    ...
    CustomDialogs.showSnackBar(
      context,
      "Error al eliminar servicio: ${e.toString().replaceAll('Exception: ', '')}",
      color: Constants.errorColor,
    );
  }
}
  ...
  Future<void> _actualizarEstadoServicio(Servicio servicio, EstadoServicio nuevoEstado) async {
    bool? confirmado = await CustomDialogs.showConfirmDialog(
      context: context,
      title: "Confirmar cambio de estado",
      content: "¿Está seguro de cambiar el estado del servicio a '${nuevoEstado.displayName}'?",
      style: const Text('')
    );
    ...
    try {
      final servicioProvider = Provider.of<ServicioProvider>(context, listen: false);
      final servicioActualizado = servicio.copyWith(estadoServicio: nuevoEstado);
      await servicioProvider.updateService(servicioActualizado.idServicio.toString(), servicioActualizado);

      if (mounted) Navigator.of(context).pop();

      CustomDialogs.showSnackBar(
        context, 
        "Estado actualizado a '${nuevoEstado.displayName}'",
        color: Constants.successColor
      );
      ...
      CustomDialogs.showSnackBar(
        context, 
        "Error al actualizar el servicio: $e",
        color: Constants.errorColor
      );
    }
  }
...
Future<void> _asignarEmpleadoAServicio(Servicio servicio, Empleado empleado) async {
  CustomDialogs.showLoadingSpinner(context);
  ...
  try {
    ...
    final servicioActualizado = await servicioProvider.asignarEmpleadoAServicio(
...
Future<void> _desasignarEmpleadoDeServicio(Servicio servicio) async {
  ...
  try {
    ...
    CustomDialogs.showSnackBar(
      context, 
      "Empleado desasignado del servicio exitosamente",
      color: Constants.successColor
    );
  ...
  List<Servicio> _filtrarServicios(List<Servicio> servicios) {
    return servicios.where((servicio) {
      ...
      if (_filtroCategoria != null && servicio.categoriaServicio != _filtroCategoria) {
        return false;
      }
  ...
  Widget _buildHeaderEstadisticas(List<Servicio> servicios, List<Empleado> empleados) {
                        ...
                        items: [
                          ...
                          ...EstadoServicio.values.map((estado) {
                            return DropdownMenuItem<EstadoServicio?>(
                              value: estado,
                              child: Text(estado.displayName, style: TextStyle(fontSize: 12)),
                            );
                          }),
                        ],
                        onChanged: (value) {
                        ...
                        items: [
                          ...
                          ...CategoriaServicio.values.map((categoria) {
                            return DropdownMenuItem<CategoriaServicio?>(
                              value: categoria,
                              child: Text(categoria.displayName, style: TextStyle(fontSize: 12)),
                            );
                          }),
                        ],
                        onChanged: (value) {
    ...
    builder: (context) {
      ...
      return Consumer<EmpleadoProvider>(
        builder: (context, empleadoProvider, child) {
          ...
          logger.i('empleados disponibles asignado: ${empleadosDisponibles.length}');
          ...
          logger.i('Construyendo lista con ${empleadosDisponibles.length} empleados');
          
          for (int i = 0; i < empleadosDisponibles.length; i++) {
            final emp = empleadosDisponibles[i];
            logger.i('Empleado para modal [$i]: ${emp.nombre} ${emp.apellidos}');
          }
                ...
                Flexible(
                  child: ListView.builder(
                    ...
                    itemBuilder: (context, index) {
                      ...
                      logger.e('🔍 [DEBUG] Construyendo ListTile $index: ${empleado.nombre}');
                      
                      return ListTile(
                        leading: CircleAvatar(
                          child: Text(empleado.nombre?.substring(0, 1).toUpperCase() ?? 'E'),
                        ),
                        title: Text('${empleado.nombre} ${empleado.apellidos}'),
                        subtitle: Text(empleado.email ?? 'Sin email'),
                        onTap: () {
                          logger.e('🔍 [DEBUG] Empleado seleccionado: ${empleado.nombre}');
...
void _navegarADetalleServicio(Servicio servicio) {
  ...
  Map<String, int> _calcularEstadisticas(List<Servicio> servicios) {
    return {
      ...
      'programados': servicios.where((s) => s.estadoServicio == EstadoServicio.programada).length,
      'progresando': servicios.where((s) => s.estadoServicio == EstadoServicio.progresando).length,
      'completados': servicios.where((s) => s.estadoServicio == EstadoServicio.completada).length,
      'cancelados': servicios.where((s) => s.estadoServicio == EstadoServicio.cancelada).length,
      'reprogramados': servicios.where((s) => s.estadoServicio == EstadoServicio.reprogramada).length,
    };
  }
    ...
    return Scaffold(
      ...
      body: Consumer2<ServicioProvider, EmpleadoProvider>(
        builder: (context, servicioProvider, empleadoProvider, child) {
          ...
          final serviciosFiltrados = _filtrarServicios(servicioProvider.servicios);
          final empleadosDisponibles = empleadoProvider.empleados
              .where((e) => !e.bajaLaboral!)
              .toList();

          return Column(
            children: [
              // NAVBAR DE ESTADISTICAS.
              _buildHeaderEstadisticas(servicioProvider.servicios, empleadosDisponibles),
                        ...
                        child: ListView.builder(
                          ...
                          itemBuilder: (context, index) {
                            ...
                            return ServicioListItem(
                                      ...
                                      onAsignarEmpleado: () => _mostrarModalAsignarEmpleado(servicio),
                            );
                          },
                        ),
                      ),
